...
### Total trips made in each type of rideable separated by member type.
```{r}
...
t_data %>% 
  ...
  labs(title = "Total of Trips by Member Type",
       ...
       fill = "Rideable Type") +
  theme_minimal()+
  facet_wrap(vars(rideable_type),scales = "free_y")
```
...
## Ploting the number of trips for each trip duration
...
p <- data %>%
  ...
  xlab("Trip Duration") +
  ylab("Count") +
  facet_wrap(vars(rideable_type,member_casual),scales = "free_y", nrow = 3)

p
```

```{r}
### return values for filter 95% of the data
data %>% 
  group_by(rideable_type, member_casual) %>% 
  summarise(
    q_95 = quantile(trip_duration, 0.95)
  )
```